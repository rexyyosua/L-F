...
class Cl extends CI_Controller {
	...
	public function create($id){
				...
				'notif'				=> 'n',
				'status_penerima'	=> 'Barang di konfirmasi',
				...
				'alamat_user'		=> $this->get_logged_alamat_user(),
				'no_tlp_user'		=> $this->get_logged_no_tlp_user(), 
				'nama_user_f'		=> $this->get_logged_nama_user($id),
				'gambar_f'			=> $this->get_logged_gambar($id),
			);
			$this->model_admin_cl->create($data_berita);
			redirect('user/rpt/found_report/'.$id);
			}	
	...
	public function update($id){
		...
		{
			$data['product'] = $this->model_admin_cl->find($id);
			$this->load->view('backend/form_edit_cl', $data);
		}else{
			if($_FILES['userfile']['name'] !=''){
				...
				{
					$data['ada'] = $this->model_admin_cl->find($id);
					$this->load->view('backend/form_edit_cl', $data);
				}else{
					...
					$this->model_admin_cl->update($id,$data_berita);
	...
	public function get_logged_alamat_user()
	{
		$hasil = $this->db
						->select('alamat')
						->where('username',$this->session->userdata('username'))
	...
	public function get_logged_no_tlp_user()
	{
		$hasil = $this->db
						->select('no_tlp')
						->where('username',$this->session->userdata('username'))
